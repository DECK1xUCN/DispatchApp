...
const pilotResolver = {
  Query: {
    pilots: async (parent: any, args: any, context: Context) => {
      const pilots = await context.prisma.pilot.findMany();
      if (!pilots) {
    ...
    pilot: async (parent: any, args: { id: string }, context: Context) => {
      const pilot = await context.prisma.pilot.findUnique({
  ...
  Mutation: {
    createPilot: async (
      ...
      const pilot = await context.prisma.pilot.create({
        data: {
          name: args.data.name,
        },
      });
    ...
    updatePilot: async (
      ...
      const pilot = await context.prisma.pilot.update({
        where: { id: parseInt(args.id) },
        data: {
          name: args.data.name,
        },
